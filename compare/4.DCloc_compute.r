...
network1=read.table(paste("rho3_0.1_",filename,"/matrix1.txt",sep=""),sep=",")

#network1 = read.table(paste(data_fold,"edgelist_HSC.txt",sep = "/"))
#colnames(network1) = c(1:ncol(network1))
#g1 = graph.adjacency(network1,directed = F)
#or another writing version
MAT1 = abs(as.matrix(network1))
rownames(MAT1) = colnames(MAT1) =c(1:100)
#E(g1)$weight = network1$V3
#MAT1 = abs(as.matrix(get.adjacency(g1,attr = "weight")))
network2=read.table(paste("rho3_0.1_",filename,"/matrix2.txt",sep=""),sep=",")
#network2 = read.table(paste(data_fold,"edgelist_MPP.txt",sep = "/"))
#g2 = graph.data.frame(network2,directed = F)
MAT2 = abs(as.matrix(network2))
rownames(MAT2)=colnames(MAT2) = c(1:100)
